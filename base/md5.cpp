...
static void md5_process_bytes (const void *buffer, size_t len, struct md5_ctx *ctx)
{
  ...
  if (len >= 64)
    {
        struct temp {char c; uint32_t x;};
#if !_STRING_ARCH_unaligned
# define UNALIGNED_P(p) (((size_t) p) % (offsetof(temp, x)) != 0)
...
static bool digest_file (const char *filename, int *binary, unsigned char *bin_result)
{
  FILE *fp;
  int err;
...
#endif
    {
      fp = fopen (filename, /* (O_BINARY && *binary ? "rb" : "r") */ "rb");
      if (fp == NULL)
        {
...
std::string md5sum(const std::string& file)
{
    ...
    unsigned char *bin_buffer = (unsigned char *)ptr_align (bin_buffer_unaligned, DIGEST_ALIGN);

    int binary = 1;
    digest_file(file.c_str(), &binary, bin_buffer);

    // std::string result;
    char temp[100];
    for (int i = 0; i < 16; i++) {
        sprintf(temp + i * 2, "%02x", bin_buffer[i]);
    }
